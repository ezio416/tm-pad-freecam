...
void RenderMenu() {
    if (UI::MenuItem(
        title + (versionSafe ? "" : "\\$AAA (disabled" + (checkingApi ? ", checking..." : "") + ")"), "",
        S_Enabled,
        versionSafe
    )) {
        S_Enabled = !S_Enabled;
...
void ToggleVanillaControls(bool b) {
...
CInputScriptPad@ GetPad() {
    ...
    auto Port = cast<CInputPortDx8>(App.InputPort);
    if (false
        or Port is null
        or Port.Script_Pads.Length == 0
    ) {
        return null;
    }
