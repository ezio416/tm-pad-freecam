...
void Render() {
    ...
    if (UI::Begin(title, UI::WindowFlags::None)) {
        ...
        Cam.m_RotateSpeed              = UI::SliderFloat ("m_RotateSpeed",              Cam.m_RotateSpeed,               0.0f,      Math::PI * 2.0f, "%.6f");  // pi/2
        ...
        Cam.m_ForcedRoll               = UI::SliderFloat ("m_ForcedRoll",               Cam.m_ForcedRoll,                0.0f,      Math::PI * 2.0f, "%.6f");  // 0.0
    ...
    if (UI::Begin(title + " trig", UI::WindowFlags::None)) {
        ...
        UI::Text("cos pit: " + cos_p);
        UI::Text("sin pit: " + sin_p);
    }

    UI::End();

    const float move_x = Pad.LeftStickY * sin_y * cos_p * 20.0f / fps;
    const float move_y = Pad.LeftStickY * -sin_p * 20.0f / fps;
    const float move_z = Pad.LeftStickY * cos_y * cos_p * 20.0f / fps;

    // if (Math::Abs(Pad.LeftStickX) > deadzone) {
    //     Cam.m_FreeVal_Loc_Translation.x -= Pad.LeftStickX * 20.0f / fps;
    //     // Cam.m_FreeVal_Loc_Translation.y
    //     // Cam.m_FreeVal_Loc_Translation.z
    // }
